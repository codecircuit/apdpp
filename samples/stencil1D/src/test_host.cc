...
void printUsage(const char* fileName) {
	cout << "Usage:" << endl
	     ...
	     << "named stencil5p_1D in the same directory." << endl;
}
...
int main(int argc, char* argv[]) {
	...
	const char* kernel_name = "stencil5p_1D";
	...
	int blocks = N * N / TILESIZE;
	
	void* args[] = { &gridA_d, &gridB_d, &N };

	if ( N * N % TILESIZE != 0) {
		cout << "***ERROR: case N * N % TILESIZE != 0 is not supported" << endl
	...
	// KERNEL LAUNCH //
	cout << "*** Launching Kernel with grid {" << blocks
	     << ", 1, 1}, block {" << threads << ", 1, 1}" << endl;

	auto kernel_begin = Clock::now();
	for (unsigned i = 0; i < T; ++i) {
		checkCudaErrors(cuLaunchKernel(function,
									   blocks, 1, 1, // Grid Size //
									   threads, 1, 1, // Block Size //
									   0, 0, args, 0));
