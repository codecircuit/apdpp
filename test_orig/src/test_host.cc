...
void printUsage(const char* fileName) {
	cout << "Usage:" << endl
	     ...
	     << "  -p <stencil code>" << endl
	     << "     You can choose between a 5 and 25 point stencil."
	     << "The program expects a stencil-kernel.ptx with a kernel" << endl
	     << "named stencil5p_2D and stencil25p_2D in the same directory." << endl;
}
...
int main(int argc, char* argv[]) {
	...
	commandLineGetInt(&stencil, "-p", argc, argv);
	...
	cout << "*  stencil = " << setw(28) << stencil << " points" << endl;
	...
	try {
		...
		cout << "*** ERROR: can't allocate grids in host memory" << endl
		     << "*** MSG: " << ba.what() << endl;
		return EXIT_FAILURE;
	}
	...
	string kernel_name = "stencil5p_2D";
	if (stencil == 25) {
		kernel_name = "stencil25p_2D";
	}
	...
	if (cuModuleGetFunction(&function, module, kernel_name.c_str()) != CUDA_SUCCESS) {
		cout << "***ERROR: while loading function " << kernel_name
		     << " from file " << ptx_file << endl;
		cuCtxDetach(context);
		return EXIT_FAILURE;
	} 
	...
	// WRITE RESULT //
	writeGrid(gpu_result, N, fname); // write the result created by the gpu //
	...
	// REPORT //
	cout << "*** Report:" << endl
	     << "**  host to device copy time = " << t_htod.count() << " s ("
	     << 2*size << " Bytes)" << endl
	     << "**  device to host copy time = " << t_dtoh.count() << " s ("
	     <<   size << " Bytes)" << endl
